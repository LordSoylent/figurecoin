...
const char * const BITCOIN_CONF_FILENAME = "figure.conf";
const char * const BITCOIN_PID_FILENAME = "figured.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "figure" is a composite category enabling all Figure-related debug output
            if(ptrCategory->count(string("figure"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FigureCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FigureCore
    // Mac: ~/Library/Application Support/FigureCore
    // Unix: ~/.figurecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FigureCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FigureCore";
#else
    // Unix
    return pathRet / ".figurecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty figure.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override figure.conf
