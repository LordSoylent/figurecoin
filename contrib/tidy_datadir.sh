...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Figure database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Figure datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Figure datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Figure 0.7 datadir."
    ;;
  3)
    echo "Detected Figure pre-0.8 datadir."
    ;;
  4)
    echo "Detected Figure 0.8 datadir."
    ;;
esac
